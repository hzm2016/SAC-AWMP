...
class LegDx(nn.Module):
    def __init__(self, params=None, simple=True):
        ...
        if params is None:
            if simple:
                ...
                self.params = Variable(torch.Tensor((9.81, 5., 0.5)))
            else:
                # gravity (g), mass (m), length (l), damping (d), gravity bias (b)
                self.params = Variable(torch.Tensor((9.81, 5., 0.5, 0., 0.)))
        else:
            self.params = params

        assert len(self.params) == 3 if simple else 5

        # cos_theta, sin_theta, dtheta
        self.goal_state = torch.Tensor([0., 1., 0.])
        self.goal_weights = torch.Tensor([1., 1., 0.])
    ...
    def forward(self, x, u):
        ...
        if not hasattr(self, 'simple') or self.simple:
            # d_theta(k+1) = d_theta(k) * dt * (u/I - mg*0.5*l*sin(theta) / I)
            # I = ml**2/3
...
if __name__ == '__main__':
    ...
    theta = 1.57
    xinit[:,0] = np.cos(theta)
    xinit[:,1] = np.sin(theta)
    x = xinit
    fig, ax = plt.subplots(figsize=(6, 6))
    for t in range(T):
        x = dx(x, u[t])
        print('x: ', x)
        fig, ax = dx.get_frame(x[0], ax=ax)
